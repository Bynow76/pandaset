...
def caculate_label(corners):
    ...
    deta1 = right_tail_bot - right_front_bot  # l
    deta2 = left_tail_bot - left_front_bot
    deta3 = right_tail_top - right_front_top
    deta4 = left_tail_top - left_front_top

    deta5 = right_front_bot - left_front_bot  # w
    deta6 = right_tail_bot - left_tail_bot
    deta7 = right_front_top - left_front_bot
    deta8 = right_tail_top - left_tail_top

    deta9 = right_front_top - right_front_bot # h
    deta10 = right_tail_top - right_tail_bot
    deta11 = left_tail_top - left_tail_bot
    deta12 = left_front_top - left_front_bot
    
    l = (np.linalg.norm(deta1,ord=2,axis=-1,keepdims=False)+np.linalg.norm(deta2,ord=2,axis=-1,keepdims=False)+
        np.linalg.norm(deta3,ord=2,axis=-1,keepdims=False)+np.linalg.norm(deta4,ord=2,axis=-1,keepdims=False))/4
    w = (np.linalg.norm(deta5,ord=2,axis=-1,keepdims=False)+np.linalg.norm(deta6,ord=2,axis=-1,keepdims=False)+
        np.linalg.norm(deta7,ord=2,axis=-1,keepdims=False)+np.linalg.norm(deta8,ord=2,axis=-1,keepdims=False))/4
    h = (np.linalg.norm(deta9,ord=2,axis=-1,keepdims=False)+np.linalg.norm(deta10,ord=2,axis=-1,keepdims=False)+
        np.linalg.norm(deta11,ord=2,axis=-1,keepdims=False)+np.linalg.norm(deta12,ord=2,axis=-1,keepdims=False))/4
...
def extract_data():
    ...
    dst_root = '/home/qiuzhongyuan/pointcloud/hesai_new'
    ...
    for index in sub_sequences:
        ...
        while True:
            ...
            # 'attributes.pedestrian_behavior', 'attributes.pedestrian_age'
            with open(os.path.join(sub_root, 'labels', "%06d.txt" % save_index), 'w') as f:
                cuboids_single_frame = cuboids_single_frame[
                    ['label', 'yaw', 'position.x', 'position.y', 'position.z', 'dimensions.x', 'dimensions.y',
                     'dimensions.z', 'cuboids.sensor_id']]
                for inde in cuboids_single_frame.index:
                    label_value = cuboids_single_frame.loc[inde].values[:]
                    sensor_id = int(label_value[-1])
                    if sensor_id > 0:  # 只用360°环视雷达
